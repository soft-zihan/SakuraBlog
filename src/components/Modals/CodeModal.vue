<template>
  <Teleport to="body">
    <div 
      ...
      class="fixed inset-0 z-[9999] bg-black/50 backdrop-blur-sm flex items-start justify-center animate-fade-in p-4 md:p-10 overflow-y-auto"
      @click.self="codeModal.closeCodeModal()"
    >
      <div class="bg-[#1e1e1e] w-full max-w-5xl max-h-[calc(100vh-2rem)] max-h-[calc(100dvh-2rem)] md:max-h-[calc(100vh-5rem)] md:max-h-[calc(100dvh-5rem)] rounded-2xl shadow-2xl border border-gray-700 flex flex-col overflow-hidden relative transform transition-all duration-300 scale-100 min-h-0">
          <!-- Modal Header -->
          ...
          <div ref="contentContainerRef" class="flex-1 min-h-0 overflow-auto custom-scrollbar p-0 bg-[#1e1e1e]">
            <pre class="p-3 md:p-6 text-sm font-mono leading-relaxed whitespace-pre hljs"><code v-html="sanitizedContent"></code></pre>
          </div>
      </div>
    </div>
  </Teleport>
...
const scrollToHighlightedLine = async () => {
  if (!codeModal.showCodeModal.value) return
  await nextTick()
  window.requestAnimationFrame(() => {
    const container = contentContainerRef.value
    if (!container) return
    const target = container.querySelector('.code-line.is-target') as HTMLElement | null
    if (!target) return
    target.scrollIntoView({ block: 'center' })
  })
}

watch(() => codeModal.showCodeModal.value, (show) => {
  if (show) scrollToHighlightedLine()
})

watch(() => codeModal.codeModalContent.value, () => {
  if (codeModal.showCodeModal.value) scrollToHighlightedLine()
})
</script>
...
:deep(.code-line.is-target) {
  background: rgba(250, 204, 21, 0.16);
}

:deep(.code-line.is-target::before) {
  color: #fbbf24;
}
</style>
