<template>
  <section class="rounded-2xl border border-white/60 dark:border-gray-700/60 bg-[#0b1220] text-slate-100 overflow-hidden">
    ...
    <div v-show="!collapsed" class="relative" :style="editorStyle">
      ...
      <pre
        ref="preRef"
        class="h-full p-4 text-[12px] leading-relaxed overflow-auto pointer-events-none select-none"
      ><code v-if="typing" class="hljs" v-text="normalizedValue"></code><code v-else class="hljs" v-html="highlighted"></code></pre>
      <textarea
        ref="textareaRef"
...
const editorAriaLabel = computed(() => (props.lang === 'zh' ? `${props.title} 编辑器` : `${props.title} editor`))

const editorHeight = computed(() => {
  const h = Number(props.editorHeight ?? 200)
  return Number.isFinite(h) && h >= 140 ? h : 200
})
...
const actionText = computed(() => {
  const label = String(props.actionLabel || '').trim()
  if (label) return label
  if (action.value === 'reset') return props.lang === 'zh' ? '还原' : 'Reset'
  if (action.value === 'clear') return props.lang === 'zh' ? '清空' : 'Clear'
  return ''
})
...
function computeHighlighted(code: string, language: string) {
  const input = String(code || '')
  try {
    const html = hljs.highlight(input, { language }).value
...
const highlighted = computed(() => computeHighlighted(normalizedValue.value, hljsLanguage.value))
...
function performAction() {
  ...
  else if (action.value === 'clear') emit('update:modelValue', '')
