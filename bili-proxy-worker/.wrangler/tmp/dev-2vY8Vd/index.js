...
__name(isPrivateHostname, "isPrivateHostname");
function safeDownloadName(input) {
...
async function handleWallpaperFile(request, env, url) {
  const target = normalizeImageUrl(url.searchParams.get("url") || "");
  if (!target) return withCors(request, env, json({ error: "Missing url" }, { status: 400 }));
  let u;
  try {
...
__name(handleWallpaperFile, "handleWallpaperFile");
...
// .wrangler/tmp/bundle-VdSAo1/middleware-insertion-facade.js
...
// .wrangler/tmp/bundle-VdSAo1/middleware-loader.entry.ts
