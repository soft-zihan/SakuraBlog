...
<template>
  <aside class="w-full flex-shrink-0 flex flex-col bg-white/80 dark:bg-gray-900/80 backdrop-blur-xl shadow-[4px_0_24px_rgba(0,0,0,0.02)] border-r border-white/60 dark:border-gray-700/50 h-full z-30 transition-all duration-300">
    ...
    <div class="px-6 py-2 md:hidden grid grid-cols-5 gap-2 shrink-0">
      <button @click="$emit('open-search')" class="flex flex-col items-center justify-center p-2 rounded-xl bg-gray-50 dark:bg-gray-800/50 text-gray-600 dark:text-gray-300 border border-gray-100 dark:border-gray-700">
        <span class="text-lg">🔍</span>
      </button>
      <button @click="$emit('open-music')" class="flex flex-col items-center justify-center p-2 rounded-xl bg-gray-50 dark:bg-gray-800/50 text-gray-600 dark:text-gray-300 border border-gray-100 dark:border-gray-700 relative">
        <span class="text-lg">🎵</span>
        <span v-if="musicStore.isPlaying" class="absolute top-1 right-1 w-2 h-2 bg-green-500 rounded-full animate-pulse"></span>
      </button>
      <button @click="$emit('toggle-theme')" class="flex flex-col items-center justify-center p-2 rounded-xl bg-gray-50 dark:bg-gray-800/50 text-gray-600 dark:text-gray-300 border border-gray-100 dark:border-gray-700">
        <span class="text-lg">{{ appStore.isDark ? '🌙' : '🌞' }}</span>
      </button>
      <button @click="$emit('open-settings')" class="flex flex-col items-center justify-center p-2 rounded-xl bg-gray-50 dark:bg-gray-800/50 text-gray-600 dark:text-gray-300 border border-gray-100 dark:border-gray-700">
      ...
      <button @click="$emit('open-download')" class="flex flex-col items-center justify-center p-2 rounded-xl bg-gray-50 dark:bg-gray-800/50 text-gray-600 dark:text-gray-300 border border-gray-100 dark:border-gray-700">
        <span class="text-lg">⬇</span>
      </button>
    </div>
    ...
    <div class="flex-1 overflow-y-auto custom-scrollbar px-4 pb-4">
      <div v-if="loading" class="flex flex-col items-center justify-center py-10 text-[var(--primary-400)]">
            ...
            <div v-if="labFolder" class="mb-4">
              ...
              <div class="space-y-1">
                <div 
                  ...
                  class="p-2 rounded-lg cursor-pointer transition-all hover:bg-white dark:hover:bg-gray-800 hover:shadow-sm border border-transparent hover:border-[var(--primary-100)] dark:hover:border-gray-700 flex items-center gap-2"
                  :class="{'bg-[var(--primary-50)] dark:bg-[var(--primary-900)]/20 border-[var(--primary-100)] dark:border-[var(--primary-800)]/30': currentFile?.path === note.path}"
                >
                ...
                <div class="space-y-2">
                    <a v-for="link in cat.items" :key="link.name" :href="link.url" target="_blank" class="block p-2 rounded-lg bg-gray-50 dark:bg-gray-800 hover:bg-white dark:hover:bg-gray-700 hover:shadow-md border border-gray-100 dark:border-gray-700 transition-all group relative overflow-hidden">
                      <div class="flex items-center gap-2 relative z-10">
                          ...
                          <span class="text-xs font-bold text-gray-700 dark:text-gray-200 group-hover:text-[var(--primary-600)] dark:group-hover:text-[var(--primary-400)] transition-colors">{{ link.name }}</span>
                      </div>
                      <div class="text-[10px] text-gray-400 dark:text-gray-500 pl-6 mt-0.5 truncate group-hover:text-[var(--primary-500)] transition-colors">{{ link.desc }}</div>
                    </a>
                </div>
              </div>
            </div>
